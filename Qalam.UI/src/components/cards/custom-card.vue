<template>
    <v-card v-bind="$attrs" :style="styles" v-on="$listeners">
        <helper-offset v-if="hasOffset" :inline="inline" :full-width="fullWidth" :offset="offset">
            <v-card v-if="!$slots.offset" :color="color" :class="`elevation-${elevation} ${customClass}`" class="v-card--material__header" :height="height">
        ...
        <v-card-text class="overflow-y-auto max-width">
            <slot />
        </v-card-text>
...
<script>
import HelperOffset from '@/components/shared/helper-offset.vue'
